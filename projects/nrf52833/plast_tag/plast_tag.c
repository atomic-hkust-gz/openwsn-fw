...
void     prob_function(uint8_t, uint8_t);

void     assemble_ibeacon_packet(void);
void     assemble_position_packet(void);
...
int mote_main(void) {
    ...
    app_vars.node_id = 1;         //    #559=1    #870=2
    ...
    sctimer_set_callback(app_vars.inner_rxtimerId, cb_inner_slot_rxtimer);
    sctimer_set_callback(app_vars.inner_txtimerId, cb_inner_slot_txtimer);
...
void assemble_ibeacon_packet(void) {
    ...
    app_vars.packet[i++]  = 0;                  //34byte, represent packet type, 0 means this packet is a sync packet.
    app_vars.packet[i++]  = 0;                  //reserve for target tag id
...
void assemble_position_packet(void) {
    ...
    app_vars.packet[i++]  = 2;                  //34byte, represent packet type, 2 means this packet is a broadcast packet.
    app_vars.packet[i++]  = app_vars.estimate_angle;                  //reserve for target tag id
    app_vars.packet[i++]  = 0x00;               // power level
}
...
void cb_endFrame(PORT_TIMER_WIDTH timestamp) {
    ...
    if (app_vars.state == APP_STATE_TX) {
        ...
        return;   // finished send a broadcast packet, end the EoF interrupt
    }
}
...
void prob_function(uint8_t numerator, uint8_t denominator) {
    uint8_t random_value = rand() % denominator; 
    app_vars.flag = (random_value < numerator);
    return;
}