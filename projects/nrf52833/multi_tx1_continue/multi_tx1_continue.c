...
int mote_main(void) {
    ...
    timer_set_callback(0, cb_inner_slot_txtimer);
    ...
    timer_capture_now(0);
    uint32_t timestamp;
    timestamp = timer_getCapturedValue(0);
    timer_schedule(0, timestamp + (16000000/5000)*0.9);
...
void cb_endFrame(PORT_TIMER_WIDTH timestamp) {
    ...
    if (app_vars.state == APP_STATE_TX & app_vars.role == BOARD_ROLE_TX_ONE) {
        ...
        uint32_t time_stamp;
        timer_capture_now(0);
        time_stamp = timer_getCapturedValue(0);
        radio_rfOff();
        app_vars.state = APP_STATE_OFF;
        //return;
        ...
        timer_schedule(0, time_stamp + (16000000/5000)*0.9);
        app_vars.packet_len = sizeof(app_vars.packet);
        assemble_ibeacon_packet(app_vars.pkt_sqn, app_vars.role);
