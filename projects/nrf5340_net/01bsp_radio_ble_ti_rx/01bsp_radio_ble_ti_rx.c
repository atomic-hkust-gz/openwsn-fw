/**
\brief This program is for using TI antenna array for scum AoA
...
int mote_main(void) {
...
#if ENABLE_DF == 1
    //antenna_TI_rx_switch_init();
    radio_configure_direction_finding_TI_antenna_switch(1);
    ...
    //radio_configure_direction_finding_TI_antenna_switch(0);
#endif
...
void cb_endFrame(PORT_TIMER_WIDTH timestamp) {
    ...
    if (app_vars.rxpk_len>LENGTH_PACKET){
        ...
        if(app_vars.rxpk_buf[2]!=0xc6 ||
           app_vars.rxpk_buf[3]!=0x80 ||
           app_vars.rxpk_buf[4]!=0x32 ||
           app_vars.rxpk_buf[5]!=0x72 ||
           app_vars.rxpk_buf[6]!=0x02 ||
           app_vars.rxpk_buf[7]!=0x00
           ){
            expectedFrame = FALSE;
        }
    }
