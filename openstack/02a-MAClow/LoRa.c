...
lora_vars_t lora_vars;
lora_stats_t lora_stats;
lora_dbg_t lora_dbg;
...
void lora_init(void) {
    ...
    lora_vars.singleChannel     = RADIO_SINGLE_CHANNEL;
#else
    ...
    lora_vars.numOfSleepSlots = 1;

    // default hopping template
    memcpy(&(lora_vars.chTemplate[0]), chTemplate_default, sizeof(lora_vars.chTemplate));
    ...
    resetStats();
    lora_stats.numDeSync = 0;

    // switch radio on
    //radio_rfOn();
    // need to add radio sleep mode
    ...
    lora_vars.timerId = opentimers_create(TIMER_TSCH, TASKPRIO_NONE);
    opentimers_scheduleAbsolute(
            lora_vars.timerId,          // timerId
            lora_vars.slotDuration,     // duration
            sctimer_readCounter(),          // reference
            TIME_TICS,                      // timetype
            isr_ieee154e_newSlot            // callback
    );
    IEEE802154_security_init();
    lora_vars.serialInhibitTimerId = opentimers_create(TIMER_INHIBIT, TASKPRIO_NONE);
}
...
PORT_TIMER_WIDTH ieee154e_asnDiff(asn_t *someASN) {
    ...
    DISABLE_INTERRUPTS();
    if (lora_vars.asn.byte4 != someASN->byte4) {
        ENABLE_INTERRUPTS();
        return (PORT_TIMER_WIDTH) 0xFFFFFFFF;;
    }

    diff = 0;
    if (lora_vars.asn.bytes2and3 == someASN->bytes2and3) {
        ENABLE_INTERRUPTS();
        return lora_vars.asn.bytes0and1 - someASN->bytes0and1;
    } else if (lora_vars.asn.bytes2and3 - someASN->bytes2and3 == 1) {
        diff = lora_vars.asn.bytes0and1;
...
void isr_ieee154e_newSlot(opentimers_id_t id) {
    ...
    lora_vars.startOfSlotReference = opentimers_getCurrentCompareValue();

    opentimers_scheduleAbsolute(
            lora_vars.timerId,                  // timerId
            TsSlotDuration,                         // duration
            lora_vars.startOfSlotReference,     // reference
            TIME_TICS,                              // timetype
            isr_ieee154e_newSlot                    // callback
    );
    lora_vars.slotDuration = TsSlotDuration;
    ...
    if (lora_vars.isSync == FALSE) {
        if (idmanager_getIsDAGroot() == TRUE) {
            changeIsSync(TRUE);
            ieee154e_resetAsn();
            lora_vars.nextActiveSlotOffset = schedule_getNextActiveSlotOffset();
        } else {
...
/**
\replacement for startOfFrame and EndOfFrame functions for LoRa

This function excutes in ISR mode.
...
bool debugPrint_asn(void) {
    asn_t output;
    output.byte4 = lora_vars.asn.byte4;
    output.bytes2and3 = lora_vars.asn.bytes2and3;
    output.bytes0and1 = lora_vars.asn.bytes0and1;
...
bool debugPrint_macStats(void) {
    ...
    openserial_printStatus(STATUS_MACSTATS, (uint8_t * ) & lora_stats, sizeof(lora_stats_t));
    return TRUE;
}
...
port_INLINE void activity_synchronize_newSlot(void) {
    ...
    if (lora_vars.state == S_SYNCRX) {
        return;
    }

    lora_vars.radioOnInit = sctimer_readCounter();
    lora_vars.radioOnThisSlot = TRUE;

    // if this is the first time I call this function while not synchronized,
    // switch on the radio in Rx mode
    if (lora_vars.state != S_SYNCLISTEN) {
        // change state
...
#if RADIO_SINGLE_CHANNEL
        lora_vars.freq = RADIO_SINGLE_CHANNEL;
#else
        lora_vars.freq = (openrandom_get16b() & 0x0F) + 11;
#endif

        // configure the radio to listen to the frequency
        radio_setFrequency(lora_vars.freq, FREQ_RX);
        ...
        lora_stats.numTicsOn += lora_vars.slotDuration;
        lora_stats.numTicsTotal += lora_vars.slotDuration;
        ...
        if (lora_vars.asn.bytes0and1 % (NUM_CHANNELS * EB_PORTION) == 0) {
            // turn off the radio (in case it wasn't yet)
            radio_rfOff();
    ...
    if ((lora_vars.state == S_SYNCLISTEN) && (lora_vars.singleChannelChanged == TRUE)) {
        // turn off the radio (in case it wasn't yet)
        radio_rfOff();

        // update record of current channel
        lora_vars.freq = calculateFrequency(lora_vars.singleChannel);
        ...
        radio_rxEnable();
        radio_rxNow();
        lora_vars.singleChannelChanged = FALSE;
    }

    // increment dummy ASN to trigger debugprint every now and then
    lora_vars.asn.bytes0and1++;

    opentimers_scheduleAbsolute(
            lora_vars.serialInhibitTimerId,     // timerId
...
port_INLINE void activity_synchronize_startOfFrame(PORT_TIMER_WIDTH capturedTime) {
    ...
    lora_vars.lastCapturedTime = capturedTime;

    // record the captured time (for sync)
    lora_vars.syncCapturedTime = capturedTime;
}
...
port_INLINE void activity_synchronize_endOfFrame(PORT_TIMER_WIDTH capturedTime) {
    ...
    lora_vars.dataReceived = openqueue_getFreePacketBuffer(COMPONENT_IEEE802154E);
    if (lora_vars.dataReceived == NULL) {
    ...
    lora_vars.dataReceived->creator = COMPONENT_IEEE802154E;
    lora_vars.dataReceived->owner = COMPONENT_IEEE802154E;
    ...
    */
    do { // this "loop" is only executed once
        ...
        lora_vars.dataReceived->payload = &(lora_vars.dataReceived->packet[FIRST_FRAME_BYTE]);
        radio_getReceivedFrame(lora_vars.dataReceived->payload,
                               (uint8_t * ) & lora_vars.dataReceived->length,
                               sizeof(lora_vars.dataReceived->packet),
                               &lora_vars.dataReceived->l1_rssi,
                               &lora_vars.dataReceived->l1_lqi,
                               &lora_vars.dataReceived->l1_crc);

        // break if packet too short
        if (lora_vars.dataReceived->length < LENGTH_CRC ||
            lora_vars.dataReceived->length > LENGTH_IEEE154_MAX) {
            // break from the do-while loop and execute abort code below

            LOG_ERROR(COMPONENT_IEEE802154E, ERR_INVALID_PACKET_FROM_RADIO,
                      (errorparameter_t) 0,
                      lora_vars.dataReceived->length);
            break;
        }

        // toss CRC (2 last bytes)
        packetfunctions_tossFooter(&lora_vars.dataReceived, LENGTH_CRC);

        // break if invalid CRC
        if (lora_vars.dataReceived->l1_crc == FALSE) {
            // break from the do-while loop and execute abort code below
            break;
        }

        // parse the IEEE802.15.4 header (synchronize, end of frame)
        ieee802154_retrieveHeader(lora_vars.dataReceived, &ieee802514_header);
        ...
        lora_vars.dataReceived->l2_frameType = ieee802514_header.frameType;
        lora_vars.dataReceived->l2_dsn = ieee802514_header.dsn;
        memcpy(&(lora_vars.dataReceived->l2_nextORpreviousHop), &(ieee802514_header.src), sizeof(open_addr_t));

        // verify that incoming security level is acceptable
        if (IEEE802154_security_acceptableLevel(lora_vars.dataReceived, &ieee802514_header) == FALSE) {
            break;
        }

        if (lora_vars.dataReceived->l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC) {
            ...
            if (!lora_vars.isSync && ieee802514_header.frameType == IEEE154_TYPE_BEACON) {
                if (!isValidJoin(lora_vars.dataReceived, &ieee802514_header)) {
                    break;
                }
        ...
        // next.
        packetfunctions_tossHeader(&lora_vars.dataReceived, ieee802514_header.headerLength);
        ...
        if (
                (
                        ...
                        ieee154e_processIEs(lora_vars.dataReceived, &lenIE)
                ) == FALSE) {
            // break from the do-while loop and execute the clean-up code below
            break;
        }
        ...
        lora_vars.radioOnTics += (sctimer_readCounter() - lora_vars.radioOnInit);

        // toss the IEs
        packetfunctions_tossHeader(&lora_vars.dataReceived, lenIE);

        // synchronize (for the first time) to the sender's EB
        synchronizePacket(lora_vars.syncCapturedTime);
        ...
        notif_receive(lora_vars.dataReceived);

        // clear local variable
        lora_vars.dataReceived = NULL;
    ...
    openqueue_freePacketBuffer(lora_vars.dataReceived);
...
port_INLINE bool ieee154e_processIEs(OpenQueueEntry_t* pkt, uint16_t* lenIE) {
    ...
    if (isValidEbFormat(pkt, lenIE) == TRUE){
        ...
        schedule_syncSlotOffset(lora_vars.slotOffset);
        lora_vars.nextActiveSlotOffset = schedule_getNextActiveSlotOffset();

        /* infer the asnOffset based on the fact that lora_vars.freq = 11 + (asnOffset + channelOffset)%16 */
        for (i = 0; i < NUM_CHANNELS; i++){
            if ((lora_vars.freq - 11) == lora_vars.chTemplate[i]){
                break;
            }
        }
        lora_vars.asnOffset = i - schedule_getChannelOffset();
        return TRUE;
    } else {
        // wrong eb format
...
port_INLINE void activity_ti1ORri1(void) {
    ...
    if (lora_vars.slotOffset == 0) {
        debugpins_frame_toggle();
    }

    // desynchronize if needed
    if (idmanager_getIsDAGroot() == FALSE) {
        if (lora_vars.deSyncTimeout > lora_vars.numOfSleepSlots) {
            lora_vars.deSyncTimeout -= lora_vars.numOfSleepSlots;
        } else {
            // Reset sleep slots
            lora_vars.numOfSleepSlots = 1;
    ...
    lora_vars.nextActiveSlotOffset = schedule_getNextActiveSlotOffset();
    if (lora_vars.slotOffset == lora_vars.nextActiveSlotOffset) {
        ...
        lora_vars.freq = calculateFrequency(schedule_getChannelOffset());

        // find the next one
        lora_vars.nextActiveSlotOffset = schedule_getNextActiveSlotOffset();
        if (idmanager_getIsSlotSkip() && idmanager_getIsDAGroot() == FALSE) {
            if (lora_vars.nextActiveSlotOffset > lora_vars.slotOffset) {
                lora_vars.numOfSleepSlots = lora_vars.nextActiveSlotOffset - lora_vars.slotOffset;
            } else {
                lora_vars.numOfSleepSlots =
                        schedule_getFrameLength() + lora_vars.nextActiveSlotOffset - lora_vars.slotOffset;
            }

            opentimers_scheduleAbsolute(
                    lora_vars.timerId,                            // timerId
                    TsSlotDuration * (lora_vars.numOfSleepSlots),   // duration
                    lora_vars.startOfSlotReference,               // reference
                    TIME_TICS,                                        // timetype
                    isr_ieee154e_newSlot                              // callback
            );
            lora_vars.slotDuration = TsSlotDuration * (lora_vars.numOfSleepSlots);

            //increase ASN by numOfSleepSlots-1 slots as at this slot is already incremented by 1
            for (i = 0; i < lora_vars.numOfSleepSlots - 1; i++) {
                incrementAsnOffset();
            }
        }
    } else {
        ...
        case CELLTYPE_TX:
            ...
            if (schedule_getOkToSend()) {
                if (packetfunctions_isBroadcastMulticast(&neighbor) == FALSE) {
                    ...
                    lora_vars.dataToSend = openqueue_macGetUnicastPacket(&neighbor);

                    if (lora_vars.dataToSend == NULL) {
                        lora_vars.dataToSend = openqueue_macGetKaPacket(&neighbor);
                    }

                    if (schedule_getShared() == FALSE) {
                        // update numCellElapsed and numCellUsed on managed Tx cell
                        if (lora_vars.dataToSend != NULL) {
                            lora_vars.dataToSend->l2_sendOnTxCell = TRUE;
                    ...
                    lora_vars.dataToSend = openqueue_macGetDIOPacket();
                    if (lora_vars.dataToSend == NULL) {
                        couldSendEB = TRUE;
                        // look for an EB packet in the queue
                        lora_vars.dataToSend = openqueue_macGetEBPacket();
                    }
                }
            }
            ...
            if (lora_vars.dataToSend == NULL) {
                ...
                // change owner
                lora_vars.dataToSend->owner = COMPONENT_IEEE802154E;
                if (couldSendEB == TRUE) { // I will be sending an EB copy synch IE -- should be Little endian?
                    ...
                    memcpy(lora_vars.dataToSend->l2_ASNpayload, &asn[0], sizeof(asn_t));
                    memcpy(lora_vars.dataToSend->l2_ASNpayload + sizeof(asn_t), &join_priority, sizeof(uint8_t));
                }
                // record that I attempt to transmit this packet
                lora_vars.dataToSend->l2_numTxAttempts++;
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
                // 1. schedule timer for loading packet
                sctimer_scheduleActionIn(ACTION_LOAD_PACKET, lora_vars.startOfSlotReference+DURATION_tt1);
                // prepare the packet for load packet action at DURATION_tt1
                // make a local copy of the frame
                packetfunctions_duplicatePacket(&lora_vars.localCopyForTransmission, lora_vars.dataToSend);

                // check if packet needs to be encrypted/authenticated before transmission
                if (lora_vars.localCopyForTransmission.l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC) { // security enabled
                    // encrypt in a local copy
                    if (IEEE802154_security_outgoingFrameSecurity(&lora_vars.localCopyForTransmission) != E_SUCCESS) {
                ...
                localCopy = &lora_vars.localCopyForTransmission;
                ...
                // configure the radio to listen to the default synchronizing channel
                radio_setFrequency(lora_vars.freq, FREQ_TX);

                // set the tx buffer address and length register.(packet is NOT loaded at this moment)
                radio_loadPacket_prepare(lora_vars.localCopyForTransmission.payload,
                                     lora_vars.localCopyForTransmission.length);
                // 2. schedule timer for sending packet
                sctimer_scheduleActionIn(ACTION_SEND_PACKET,  lora_vars.startOfSlotReference+DURATION_tt2);
                // 3. schedule timer radio tx watchdog
                sctimer_scheduleActionIn(ACTION_SET_TIMEOUT, lora_vars.startOfSlotReference+DURATION_tt3);
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
            // arm rt1
            sctimer_scheduleActionIn(ACTION_RADIORX_ENABLE,lora_vars.startOfSlotReference+DURATION_rt1);
            radio_rxPacket_prepare();
            // 2. schedule timer for starting
            sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,  lora_vars.startOfSlotReference+DURATION_rt2);
...
port_INLINE void activity_ti2(void) {
    ...
    if (packetfunctions_reserveFooter(&localCopy, 2) == E_FAIL){
        ...
        lora_vars.dataToSend->l2_retriesLeft = 1;
        endSlot();
        return;
    }
    ...
    radio_loadPacket(lora_vars.localCopyForTransmission.payload, lora_vars.localCopyForTransmission.length);
#endif
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm tt4
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,lora_vars.startOfSlotReference+DURATION_tt4);
#else
    // arm tt4
    opentimers_scheduleAbsolute(
            lora_vars.timerId,                            // timerId
...
port_INLINE void activity_ti5(PORT_TIMER_WIDTH capturedTime) {
    ...
    if (packetfunctions_isBroadcastMulticast(&lora_vars.dataToSend->l2_nextORpreviousHop) == TRUE) {
        listenForAck = FALSE;
    } else {
        listenForAck = TRUE;
    }
    ...
    if (listenForAck == TRUE) {
        ...
        // arm tt5
        sctimer_scheduleActionIn(ACTION_RADIORX_ENABLE,lora_vars.startOfSlotReference+DURATION_tt5);
        // set receiving buffer address (radio is NOT enabled at this moment)
        radio_rxPacket_prepare();
        // 2. schedule timer for starting receiving
        sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,lora_vars.startOfSlotReference+DURATION_tt6);
        ...
        schedule_indicateTx(&lora_vars.asn, TRUE);
        // indicate to upper later the packet was sent successfully
        notif_sendDone(lora_vars.dataToSend, E_SUCCESS);
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm tt7
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,lora_vars.startOfSlotReference+DURATION_tt7);
#else
    // arm tt7
    opentimers_scheduleAbsolute(
            lora_vars.timerId,                            // timerId
...
port_INLINE void activity_tie5(void) {
    ...
    schedule_indicateTx(&lora_vars.asn, FALSE);

    // decrement transmits left counter
    lora_vars.dataToSend->l2_retriesLeft--;

    if (lora_vars.dataToSend->l2_retriesLeft == 0) {
        // indicate tx fail if no more retries left
        notif_sendDone(lora_vars.dataToSend, E_FAIL);
    } else {
        // return packet to the virtual COMPONENT_SIXTOP_TO_IEEE802154E component
        lora_vars.dataToSend->owner = COMPONENT_SIXTOP_TO_IEEE802154E;
    }
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm tt8
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,lora_vars.startOfSlotReference+DURATION_tt8);
#else
    // arm tt8
    opentimers_scheduleAbsolute(
            lora_vars.timerId,                            // timerId
...
port_INLINE void activity_ti9(PORT_TIMER_WIDTH capturedTime) {
    ...
    lora_vars.ackReceived = openqueue_getFreePacketBuffer(COMPONENT_IEEE802154E);
    if (lora_vars.ackReceived == NULL) {
        // log the error
        LOG_ERROR(COMPONENT_IEEE802154E, ERR_NO_FREE_PACKET_BUFFER, (errorparameter_t) 0, (errorparameter_t) 0);
        // abort
        endSlot();
    ...
    // declare ownership over that packet
    lora_vars.ackReceived->creator = COMPONENT_IEEE802154E;
    lora_vars.ackReceived->owner = COMPONENT_IEEE802154E;
    ...
    */
    do { // this "loop" is only executed once
        ...
        lora_vars.ackReceived->payload = &(lora_vars.ackReceived->packet[FIRST_FRAME_BYTE]);
        radio_getReceivedFrame(
                lora_vars.ackReceived->payload,
                (uint8_t * ) & lora_vars.ackReceived->length,
                sizeof(lora_vars.ackReceived->packet),
                &lora_vars.ackReceived->l1_rssi,
                &lora_vars.ackReceived->l1_lqi,
                &lora_vars.ackReceived->l1_crc
        );

        // break if wrong length
        if (lora_vars.ackReceived->length < LENGTH_CRC || lora_vars.ackReceived->length > LENGTH_IEEE154_MAX) {
            // break from the do-while loop and execute the clean-up code below
            LOG_ERROR(COMPONENT_IEEE802154E, ERR_INVALID_PACKET_FROM_RADIO,
                      (errorparameter_t) 1,
                      lora_vars.ackReceived->length);

            break;
        }

        // toss CRC (2 last bytes)
        packetfunctions_tossFooter(&lora_vars.ackReceived, LENGTH_CRC);

        // break if invalid CRC
        if (lora_vars.ackReceived->l1_crc == FALSE) {
            // break from the do-while loop and execute the clean-up code below
            break;
        }

        // parse the IEEE802.15.4 header (RX ACK)
        ieee802154_retrieveHeader(lora_vars.ackReceived, &ieee802514_header);
        ...
        // store header details in packet buffer
        lora_vars.ackReceived->l2_frameType = ieee802514_header.frameType;
        lora_vars.ackReceived->l2_dsn = ieee802514_header.dsn;
        memcpy(&(lora_vars.ackReceived->l2_nextORpreviousHop), &(ieee802514_header.src), sizeof(open_addr_t));

        // verify that incoming security level is acceptable
        if (IEEE802154_security_acceptableLevel(lora_vars.ackReceived, &ieee802514_header) == FALSE) {
            break;
        }

        // check the security level of the ACK frame and decrypt/authenticate
        if (lora_vars.ackReceived->l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC) {
            if (IEEE802154_security_incomingFrame(lora_vars.ackReceived) != E_SUCCESS) {
                break;
            }
        }

        // toss the IEEE802.15.4 header
        packetfunctions_tossHeader(&lora_vars.ackReceived, ieee802514_header.headerLength);

        // break if invalid ACK
        if (isValidAck(&ieee802514_header, lora_vars.dataToSend) == FALSE) {
            // break from the do-while loop and execute the clean-up code below
            break;
        }

        if (idmanager_getIsDAGroot() == FALSE &&
            icmpv6rpl_isPreferredParent(&(lora_vars.ackReceived->l2_nextORpreviousHop))) {
    ...
    openqueue_freePacketBuffer(lora_vars.ackReceived);

    // clear local variable
    lora_vars.ackReceived = NULL;
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm rt3
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,lora_vars.startOfSlotReference+DURATION_rt3);
#else
    // arm rt3
    opentimers_scheduleAbsolute(
            lora_vars.timerId,                            // timerId
...
port_INLINE void activity_ri4(PORT_TIMER_WIDTH capturedTime) {
    ...
    lora_vars.syncCapturedTime = capturedTime;
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,lora_vars.startOfSlotReference+DURATION_rt4);
#else
    opentimers_scheduleAbsolute(
            lora_vars.timerId,                            // timerId
...
port_INLINE void activity_ri5(PORT_TIMER_WIDTH capturedTime) {
    ...
    do { // this "loop" is only executed once
        ...
        lora_vars.dataReceived->payload = &(lora_vars.dataReceived->packet[FIRST_FRAME_BYTE]);
        radio_getReceivedFrame(
                lora_vars.dataReceived->payload,
                (uint8_t * ) & lora_vars.dataReceived->length,
                sizeof(lora_vars.dataReceived->packet),
                &lora_vars.dataReceived->l1_rssi,
                &lora_vars.dataReceived->l1_lqi,
        ...
        if (lora_vars.dataReceived->length < LENGTH_CRC ||
            ...
            LOG_ERROR(COMPONENT_IEEE802154E, ERR_INVALID_PACKET_FROM_RADIO,
                      (errorparameter_t) lora_vars.dataReceived->length,
                      (errorparameter_t) 2);
            break;
        }
        ...
        lora_vars.dataReceived->l2_frameType = ieee802514_header.frameType;
        lora_vars.dataReceived->l2_dsn = ieee802514_header.dsn;
        lora_vars.dataReceived->l2_IEListPresent = ieee802514_header.ieListPresent;
        memcpy(&(lora_vars.dataReceived->l2_nextORpreviousHop), &(ieee802514_header.src), sizeof(open_addr_t));
        ...
        if (lora_vars.dataReceived->l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC) {
            if (IEEE802154_security_isConfigured()) {
                if (IEEE802154_security_incomingFrame(lora_vars.dataReceived) != E_SUCCESS) {
                    break;
                }
            }
                // bypass authentication of beacons during join process
            else if (lora_vars.dataReceived->l2_frameType == IEEE154_TYPE_BEACON) { // not joined yet
                packetfunctions_tossFooter(&lora_vars.dataReceived,
                                           lora_vars.dataReceived->l2_authenticationLength);
            } else {
                break;
            }
        }
        ...
        if (
            ...
            if (ieee154e_processIEs(lora_vars.dataReceived, &lenIE) == FALSE) {
        ...
        if (icmpv6rpl_getPreferredParentEui64(&parentAddress)) {
            if (packetfunctions_sameAddress(&ieee802514_header.src, &parentAddress)) {
                lora_vars.receivedFrameFromParent = TRUE;
            }
        }

        // record the timeCorrection and print out at end of slot
        lora_vars.dataReceived->l2_timeCorrection = (PORT_SIGNED_INT_WIDTH)(
                (PORT_SIGNED_INT_WIDTH) TsTxOffset - (PORT_SIGNED_INT_WIDTH) lora_vars.syncCapturedTime);

        // check if ack requested
        if (ieee802514_header.ackRequested == 1 && lora_vars.isAckEnabled == TRUE) {
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
            // get a buffer to put the ack to send in
            lora_vars.ackToSend = openqueue_getFreePacketBuffer(COMPONENT_IEEE802154E);
            if (lora_vars.ackToSend == NULL) {
                // log the error
                LOG_ERROR(COMPONENT_IEEE802154E, ERR_NO_FREE_PACKET_BUFFER, (errorparameter_t)0, (errorparameter_t)0);
            ...
            // declare ownership over that packet
            lora_vars.ackToSend->creator = COMPONENT_IEEE802154E;
            lora_vars.ackToSend->owner = COMPONENT_IEEE802154E;
            ...
            lora_vars.timeCorrection = (PORT_SIGNED_INT_WIDTH)(
                    (PORT_SIGNED_INT_WIDTH) TsTxOffset - (PORT_SIGNED_INT_WIDTH) lora_vars.syncCapturedTime);

            // prepend the IEEE802.15.4 header to the ACK
            lora_vars.ackToSend->l2_frameType = IEEE154_TYPE_ACK;
            lora_vars.ackToSend->l2_dsn = lora_vars.dataReceived->l2_dsn;

            // To send ACK, we use the same security level (including NOSEC) and keys
            // that were present in the DATA packet.
            lora_vars.ackToSend->l2_securityLevel = lora_vars.dataReceived->l2_securityLevel;
            lora_vars.ackToSend->l2_keyIdMode = lora_vars.dataReceived->l2_keyIdMode;
            lora_vars.ackToSend->l2_keyIndex = lora_vars.dataReceived->l2_keyIndex;

            ieee802154_prependHeader(
                    lora_vars.ackToSend,
                    lora_vars.ackToSend->l2_frameType,
                    FALSE,//no payloadIE in ack
                    lora_vars.dataReceived->l2_dsn,
                    &(lora_vars.dataReceived->l2_nextORpreviousHop)
            );

            // if security is enabled, encrypt directly in OpenQueue as there are no retransmissions for ACKs
            if (lora_vars.ackToSend->l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC) {
                if (IEEE802154_security_outgoingFrameSecurity(lora_vars.ackToSend) != E_SUCCESS) {
                    openqueue_freePacketBuffer(lora_vars.ackToSend);
                    endSlot();
                    return;
                }
            }
            // space for 2-byte CRC
            if (packetfunctions_reserveFooter(&lora_vars.ackToSend, 2) == E_FAIL){
                endSlot();
                return;
            }
            // 1. schedule timer for loading packet
            sctimer_scheduleActionIn(ACTION_LOAD_PACKET, lora_vars.startOfSlotReference + DURATION_rt5);
            // set tx buffer address and length to prepare loading packet (packet is NOT loaded at this moment)
            radio_loadPacket_prepare(lora_vars.ackToSend->payload,
                                     lora_vars.ackToSend->length);
            sctimer_scheduleActionIn(ACTION_SEND_PACKET, lora_vars.startOfSlotReference + DURATION_rt6);
            // 2. schedule timer for radio tx watchdog
            sctimer_scheduleActionIn(ACTION_SET_TIMEOUT, lora_vars.startOfSlotReference + DURATION_rt7);
            ...
            if (
                    ...
                    (
                            icmpv6rpl_isPreferredParent(&(lora_vars.dataReceived->l2_nextORpreviousHop)) ||
...
port_INLINE void activity_ri6(void) {
    ...
    ieee802154_prependHeader(lora_vars.ackToSend,
                             lora_vars.ackToSend->l2_frameType,
                             FALSE,//no payloadIE in ack
                             lora_vars.dataReceived->l2_dsn,
                             &(lora_vars.dataReceived->l2_nextORpreviousHop)
    ...
    // load the packet in the radio's Tx buffer
    radio_loadPacket(lora_vars.ackToSend->payload, lora_vars.ackToSend->length);
#endif
...
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
    // arm rt8
    sctimer_scheduleActionIn(ACTION_SET_TIMEOUT,lora_vars.startOfSlotReference+DURATION_rt8);
#else
    // arm rt8
    opentimers_scheduleAbsolute(
            lora_vars.timerId,                            // timerId
...
port_INLINE void incrementAsnOffset(void) {
    ...
    // increment the asn
    lora_vars.asn.bytes0and1++;
    if (lora_vars.asn.bytes0and1 == 0) {
        lora_vars.asn.bytes2and3++;
        if (lora_vars.asn.bytes2and3 == 0) {
            lora_vars.asn.byte4++;
        }
    }
    ...
    if (frameLength == 0) {
        lora_vars.slotOffset++;
    } else {
        lora_vars.slotOffset = (lora_vars.slotOffset + 1) % frameLength;
    }
    lora_vars.asnOffset = (lora_vars.asnOffset + 1) % NUM_CHANNELS;
}

port_INLINE void ieee154e_resetAsn(void) {
    // reset slotoffset
    lora_vars.slotOffset = 0;
    lora_vars.asnOffset = 0;
    // reset asn
    lora_vars.asn.byte4 = 0;
    lora_vars.asn.bytes2and3 = 0;
    lora_vars.asn.bytes0and1 = 0;
}

//from upper layer that want to send the ASN to compute timing or latency
port_INLINE void ieee154e_getAsn(uint8_t *array) {
    array[0] = (lora_vars.asn.bytes0and1 & 0xff);
    array[1] = (lora_vars.asn.bytes0and1 / 256 & 0xff);
    array[2] = (lora_vars.asn.bytes2and3 & 0xff);
    array[3] = (lora_vars.asn.bytes2and3 / 256 & 0xff);
    array[4] = lora_vars.asn.byte4;
}

port_INLINE uint16_t ieee154e_getTimeCorrection(void) {
    int16_t returnVal;

    returnVal = (uint16_t)(lora_vars.timeCorrection);
    return returnVal;
}

void ieee154e_getTicsInfo(uint32_t *numTicsOn, uint32_t *numTicsTotal) {
    *numTicsOn = (uint32_t)(lora_stats.numTicsOn);
    *numTicsTotal = (uint32_t)(lora_stats.numTicsTotal);
}

port_INLINE void joinPriorityStoreFromEB(uint8_t jp) {
    lora_vars.dataReceived->l2_joinPriority = jp;
    lora_vars.dataReceived->l2_joinPriorityPresent = TRUE;
}
...
port_INLINE void asnStoreFromEB(uint8_t *asn) {
    ...
    // store the ASN
    lora_vars.asn.bytes0and1 = asn[0] + 256 * asn[1];
    lora_vars.asn.bytes2and3 = asn[2] + 256 * asn[3];
    lora_vars.asn.byte4 = asn[4];
}
...
port_INLINE void ieee154e_syncSlotOffset(void) {
    ...
    slotOffset = lora_vars.asn.byte4;
    slotOffset = slotOffset % frameLength;
    slotOffset = slotOffset << 16;
    slotOffset = slotOffset + lora_vars.asn.bytes2and3;
    slotOffset = slotOffset % frameLength;
    slotOffset = slotOffset << 16;
    slotOffset = slotOffset + lora_vars.asn.bytes0and1;
    slotOffset = slotOffset % frameLength;

    lora_vars.slotOffset = (slotOffset_t) slotOffset;

    schedule_syncSlotOffset(lora_vars.slotOffset);
    lora_vars.nextActiveSlotOffset = schedule_getNextActiveSlotOffset();
    /*
    infer the asnOffset based on the fact that
    lora_vars.freq = 11 + (asnOffset + channelOffset)%16
    */
    for (i = 0; i < NUM_CHANNELS; i++) {
        if ((lora_vars.freq - 11) == lora_vars.chTemplate[i]) {
            break;
...
void synchronizePacket(PORT_TIMER_WIDTH timeReceived) {
    ...
    currentValue = opentimers_getValue() - lora_vars.startOfSlotReference;
    currentPeriod = lora_vars.slotDuration;
    ...
    lora_vars.slotDuration = newPeriod;
    // indicate time correction to adaptive sync module
#if OPENWSN_ADAPTIVE_SYNC_C
    adaptive_sync_indicateTimeCorrection(timeCorrection,lora_vars.dataReceived->l2_nextORpreviousHop);
#endif
    // reset the de-synchronization timeout
    lora_vars.deSyncTimeout = DESYNCTIMEOUT;

    // log a large timeCorrection
    if (
            lora_vars.isSync == TRUE &&
            (
...
#if OPENWSN_ADAPTIVE_SYNC_C
    // indicate time correction to adaptive sync module
    adaptive_sync_indicateTimeCorrection((-timeCorrection),lora_vars.ackReceived->l2_nextORpreviousHop);
#endif
    // log a large timeCorrection
    if (lora_vars.isSync == TRUE && (
            timeCorrection < -LIMITLARGETIMECORRECTION || timeCorrection > LIMITLARGETIMECORRECTION
            )) {
        LOG_WARNING(COMPONENT_IEEE802154E, ERR_LARGE_TIMECORRECTION,
                (errorparameter_t) timeCorrection,
...
void notif_sendDone(OpenQueueEntry_t *packetSent, owerror_t error) {
    ...
    memcpy(&packetSent->l2_asn, &lora_vars.asn, sizeof(asn_t));
...
void notif_receive(OpenQueueEntry_t *packetReceived) {
    // record the current ASN
    memcpy(&packetReceived->l2_asn, &lora_vars.asn, sizeof(asn_t));
...
port_INLINE void resetStats(void) {
    lora_stats.numSyncPkt = 0;
    lora_stats.numSyncAck = 0;
    lora_stats.minCorrection = 127;
    lora_stats.maxCorrection = -127;
    lora_stats.numTicsOn = 0;
    lora_stats.numTicsTotal = 0;
    // do not reset the number of de-synchronizations
}

void updateStats(PORT_SIGNED_INT_WIDTH timeCorrection) {
    // update minCorrection
    if (timeCorrection < lora_stats.minCorrection) {
        lora_stats.minCorrection = timeCorrection;
    }
    // update maxConnection
    if (timeCorrection > lora_stats.maxCorrection) {
        lora_stats.maxCorrection = timeCorrection;
    }
}
...
port_INLINE uint8_t calculateFrequency(uint8_t channelOffset) {
    if (lora_vars.singleChannel >= 11 && lora_vars.singleChannel <= 26 ) {
        return lora_vars.singleChannel; // single channel
    } else {
        // channel hopping enabled, use the channel depending on hopping template
        return 11 + lora_vars.chTemplate[(lora_vars.asnOffset + channelOffset) % NUM_CHANNELS];
    }
}
...
void endSlot(void) {
    ...
    if (lora_vars.radioOnThisSlot == TRUE) {
        lora_vars.radioOnTics += (sctimer_readCounter() - lora_vars.radioOnInit);
    }
#ifdef SLOT_FSM_IMPLEMENTATION_MULTIPLE_TIMER_INTERRUPT
...
#endif
    // reset capturedTimes
    lora_vars.lastCapturedTime = 0;
    lora_vars.syncCapturedTime = 0;

    // compute duty cycle.
    lora_stats.numTicsOn += lora_vars.radioOnTics;//accumulate and tics the radio is on for that window
    lora_stats.numTicsTotal += lora_vars.slotDuration;//increment total tics by timer period.

    if (lora_stats.numTicsTotal > DUTY_CYCLE_WINDOW_LIMIT) {
        lora_stats.numTicsTotal = lora_stats.numTicsTotal >> 1;
        lora_stats.numTicsOn = lora_stats.numTicsOn >> 1;
    }

    // clear vars for duty cycle on this slot
    lora_vars.radioOnTics = 0;
    lora_vars.radioOnThisSlot = FALSE;
    ...
    lora_vars.receivedFrameFromParent = FALSE;
    ...
    if (
            schedule_getSlottOffset() == lora_vars.slotOffset &&
    ...
    if (DURATION_si + lora_vars.startOfSlotReference - opentimers_getValue() < lora_vars.slotDuration) {
        openserial_inhibitStop(); // end of slot
    }
}

bool ieee154e_isSynch(void) {
    return lora_vars.isSync;
}
